...
def html_svg_print_matrix(p_rows: int, p_cols: int, p_heading: str, p_matrix: list[list[int]]) -> None:
    """
    ...
    for l_row in range(p_rows+2):
        ...
        for l_col in range(p_cols+2):
            if (p_matrix[l_row][l_col] == 0):
                l_class: str = 'destination'
            elif (p_matrix[l_row][l_col] == ((p_rows*p_cols)+1)):
                l_class: str = 'empty'
            elif (p_matrix[l_row][l_col] == ((p_rows*p_cols)+2)):
                l_class: str = 'blocked'
            else:
                l_class: str = 'full'
            print(f'<rect x="{l_x}" y="{l_y}" height="{l_y_step}" width="{l_x_step}" class="{l_class}"/>')
            print(f'<text x="{l_x+l_x_step/2}" y="{l_y+25}" class="path">{p_matrix[l_row][l_col]}</text>')
            l_x += l_x_step
        l_y += l_y_step
...
def main(p_argv: list[str]) -> None:
    ...
    if validate_dimensions(l_rows, l_cols, l_matrix):
        ...
        l_loop: int = 0
        html_write_headings()
        html_svg_print_matrix(l_rows, l_cols, f'Starting', l_matrix)
        l_time0 = perf_counter()
        while (l_changes > 0):
            l_changes: int = update_matrix(l_rows, l_cols, l_matrix)
            if (l_changes > 0):
                html_svg_print_matrix(l_rows, l_cols, f'After loop {l_loop}', l_matrix)
            l_loop += 1
